// OptionBalloon.cpp: implementation of the COptionBalloon class.
//
//////////////////////////////////////////////////////////////////////
...
BEGIN_MESSAGE_MAP(COptionBalloon, CBalloonWnd)
	//{{AFX_MSG_MAP(COptionBalloon)
	ON_WM_PAINT()
	ON_WM_CLOSE()
...
/////////////////////////////////////////////////////////////////////////////
// COptionBalloon message handlers


void COptionBalloon::OnPaint() 
{
...
BOOL COptionBalloon::SetUpControls(int nButtons, int nRadios, DWORD dwFlags )
{
	CleanUp();
	...
	if ( dwFlags & BI_TEXTBOX )
	{
		...
		m_pEdit->SetWindowText(MAINDLG->GetSink()->strWeb);
	}

	// Radio
	for (int i=0; i< nRadios; i++)
	{
		CString strRadio[] = {_T("코드"),_T("리소스")};	// Window caption
		pButton = new CAgentButton;	// Alloc

		// Create, init, etc.
		if ( !pButton->Create( strRadio[i], WS_VISIBLE | WS_CHILD | WS_TABSTOP | BS_AUTORADIOBUTTON | BS_TEXT
	...
	if ( dwFlags & BI_CHECKBOX )
	{
		...
		// Create, init, etc.
		if ( !m_pCheckBox->Create( "웹 주소 자동 입력", WS_VISIBLE | WS_CHILD | WS_TABSTOP | BS_AUTOCHECKBOX,
		...
		m_pCheckBox->SetCheck(MAINDLG->GetSink()->bChecked ? 1 : 0);
	}
...
void COptionBalloon::SetPosition( DWORD dwFlags )
{
	CPaintDC dc(this);
	...
	// Set rects
	rectClient.SetRect( 0, 8, 200, 10000 );
	...
	if ( m_strTitle != "" )
	{
		...
		rectClient.top += rectDraw.Height() + 4;		// Add margin
	...
	if ( m_strText != "" )
	{
		...
		rectClient.top += rectDraw.Height() + 8;		// Add margin
		if ( rectClient.right < rectDraw.right ) rectClient.right = rectDraw.right;
	}
	...
	{
		// Move radio
		m_arRadios[i]->SetWindowPos( NULL, 0, rectClient.top, rectClient.right, 0, SWP_NOZORDER );
	...
	if ( m_pCheckBox )
	{
		...
		ScreenToClient( &rectWindow );
		rectClient.top = rectWindow.bottom + 8;	// Add margin
	}
...
long COptionBalloon::Show ( HWND hParent, DWORD dwFlags )
{
	int nRet = -1;
	// First calc position
...
void COptionBalloon::Move ( RECT rectParent )
{
	m_rectParent = rectParent;
	CalcAndMove( rectParent );
}

BOOL COptionBalloon::SetButtonText(int nIndex, CString strText)
{
...
BOOL COptionBalloon::SetRadioText( int nIndex, CString strText )
{
...
BOOL COptionBalloon::SetCheckBoxText( CString strText )
{
...
void COptionBalloon::OnClose() 
{
	...
	if ( OptionNoModal )
		OptionNoModal ( -1, m_lExtra, m_lExtra2 );
}
LRESULT COptionBalloon::OnChild(WPARAM wParam, LPARAM lParam)
{
...
CloseBalloon:
	...
	// Call callback function
	if ( OptionNoModal )
		OptionNoModal ( nID, m_lExtra, m_lExtra2 );

	return 0;
}

CString COptionBalloon::GetTextBox()
{
	CString strText;
...
BOOL COptionBalloon::GetCheckBoxState()
{
	if ( m_pCheckBox )
...
void COptionBalloon::SetCheckBoxState(BOOL bCheck)
{
	if (m_pCheckBox)
		m_pCheckBox->SetCheck(bCheck ? 1 : 0);
}

long COptionBalloon::GetCheckedRadio()
{
